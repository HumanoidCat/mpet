...
| MFCC/pitch/FFT **implementados a mano** y verificados contra la definición matemática | usar librería directamente | Es un curso de Señales y Sistemas: implementar la STFT, el banco de filtros mel y YIN demuestra los conceptos. La correctitud se prueba contra la definición directa de la DFT y contra propiedades exactas (Parseval, linealidad, simetría conjugada, desplazamiento), que es más fuerte que comparar contra otra implementación y no añade dependencias (ver D-07). |
